...
for FileNumber=RELAX_epoching_cfg.FilesToProcess(1,1:size(RELAX_epoching_cfg.FilesToProcess,2))
    ...
    % Select channels to be epoched
    EEG=pop_select(EEG,'channel',RELAX_epoching_cfg.EpochChannels);
    ...
    if strcmp(RELAX_epoching_cfg.BL_correction_method,'regression')
        ...
        if RELAX_epoching_cfg.NumberOfFactors==1
            if RELAX_epoching_cfg.NumberOfConditions==2
            ...
            if RELAX_epoching_cfg.NumberOfConditions>2
                [EEG]=RELAX_RegressionBL_Correction(EEG,RELAX_epoching_cfg,'Factor_1_Level_1',RELAX_epoching_cfg.BL_correction_Factor_1_Level_1,'Factor_1_Levels',RELAX_epoching_cfg.BL_correction_Factor_1_Levels); % if a 2 condition design, this will code triggers other than 'Go' as -1, and Go as 1
            end
        end
    ...
    % Display rejected epochs
    if RELAX_epoching_cfg.PlotEpochRejection

        colormatrej = repmat([1.0000 0.9765 0.5294], sum(EEG.reject.rejglobal),1);
        chanrej = repmat(zeros(1, EEG.nbchan),sum(EEG.reject.rejglobal),1);
        rejepoch = ([EEG.event(find(EEG.reject.rejglobal)).epoch]'-1)*EEG.pnts;

        matrixrej   = [rejepoch rejepoch+EEG.pnts colormatrej chanrej];
        eegplot(EEG.data, 'title',FileName,'winrej', matrixrej,'srate',EEG.srate, ...
            'events',EEG.event,'winlength', 20, 'spacing', 50); % call EEGPLOT GUI
    end
...
if strcmp(RELAX_epoching_cfg.InterpolateRejectedChannels,'yes')
    ...
    for x=1:size(MissingLabels,2)
        MatCompatible_Participant_IDs{1,MissingLabels(x)}=['NotEpoched',num2str(x)];
    end
