...
function [continuousEEG, d] = RELAX_metrics_final_SER_and_ARR(rawEEG, continuousEEG)
    ...
    % 修改：用NaN替换坏段而不是删除，以保持数据长度一致
    % 原来：rawEEG = eeg_eegrej( rawEEG, continuousEEG.RELAX.ExtremelyBadPeriodsForDeletion);

    % 新方法：在rawEEG中也用NaN替换坏段
    if ~isempty(continuousEEG.RELAX.ExtremelyBadPeriodsForDeletion)
        for i = 1:size(continuousEEG.RELAX.ExtremelyBadPeriodsForDeletion, 1)
            bad_start = continuousEEG.RELAX.ExtremelyBadPeriodsForDeletion(i, 1);
            bad_end = continuousEEG.RELAX.ExtremelyBadPeriodsForDeletion(i, 2);
            rawEEG.data(:, bad_start:bad_end) = NaN;
        end
    end
    ...
    % Then by using that template to replace periods in the masks with NaN, and
    ...
    if isfield(continuousEEG.RELAX,'NoiseMaskFullLengthR1')
        ...
        NoiseMaskFullLengthR1(:,reject == 1) = NaN;  % 改为NaN而不是删除
    ...
    if isfield(continuousEEG.RELAX,'NoiseMaskFullLengthR2')
        ...
        NoiseMaskFullLengthR2(:,reject == 1) = NaN;  % 改为NaN而不是删除
    ...
    if isfield(continuousEEG.RELAX,'NoiseMaskFullLengthR3')
        ...
        NoiseMaskFullLengthR3(:,reject == 1) = NaN;  % 改为NaN而不是删除
