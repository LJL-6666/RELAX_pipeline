...
function [Regression_BL_Corrected_EEG]=RELAX_RegressionBL_Correction(EEG,RELAX_epoching_cfg,varargin)
    ...
    %% check inputs
    Factor1_present=0;
    Factor1_MultiLevels=0;

    for k = 1:length(varargin)
        if strcmpi(varargin{k},'Factor_1_Level_1')
            Factor1_present=1;
            Factor_1_Level_1_Codes = varargin{k+1}; 
            if strcmpi(varargin{k+2},'Factor_1_Levels')
                Factor_1_Levels_Codes = varargin{k+3}; 
                Factor1_MultiLevels=1;
            end
            varargin{k+1}=[]; 
            varargin{k}=[];
            varargin{k+3}=[]; 
    ...
    for e=1:size(EEG.data,3) % for each event:
        ...
        if Factor1_present==1
            if Factor1_MultiLevels==1
                nconds = length(Factor_1_Levels_Codes);
                for icond = 1:nconds
                    if contains(EEG.event(e).type, Factor_1_Levels_Codes{icond}) == 1
                        confounds(e, icond+1) = 1; % 为当前条件设置哑变量
                    end
                end
                if contains(EEG.event(e).type,Factor_1_Level_1_Codes)==1 
                   confounds(e,2:nconds+1)=-1; 
                end
            else
                nconds = 1;
                if contains(EEG.event(e).type,Factor_1_Level_1_Codes)==1 
                   confounds(e,2)=1; % code SadGo as [-1 1]
    ...
    allbetas = NaN(size(EEG.data,1),nconds*2+2,size(EEG.data, 2));
    for c=1:size(EEG.data,1) % c = channels, for each channel
        ...
        % 添加交互项：基线均值 × 条件编码（交互效应）
        if Factor1_present==1
            if Factor1_MultiLevels==0
                confounds(:,size(confounds,2)+1) = confounds(:,1) .* confounds(:,2); % 与条件1的交互
            else 
                for icond = 1:nconds
                    confounds(:, nconds+2+icond) = confounds(:,1).*confounds(:,1+icond); % 基线与各条件交互项
                end
            end
        end
        ...
        % end
        for s = 1:size(Beta,2)
            % 将基线均值及其交互项的贡献都包括在 model 中
            model(1,s,:) = squeeze(confounds(:, [1,nconds+3:end])) * squeeze(Beta([1,nconds+3:end], s));
        end
    ...
    % % 
    % plot(squeeze(allbetas(13,1,:)))
    % hold on;
    % plot(squeeze(allbetas(13,nconds+2,:)))
    % plot(squeeze(allbetas(13,nconds+2,:))-sum(squeeze(allbetas(13,2:(nconds+1),:)),1)')
    % for icond=1:nconds
    % % plot(squeeze(allbetas(13,3,:)))
    %     % plot(squeeze(allbetas(13,nconds+2,:))+squeeze(allbetas(13,icond+1,:)))
    %     plot(squeeze(allbetas(13,icond+8,:)),'linewidth',1.3)
    % 
    % end
    % legend_plot7 = {'Neutral','Disgust','Fear','Sadness','Amusement','Inspiration','Tenderness'};
    % legend([{'baseline','Beta'},legend_plot7])
end

for i =1:length({EEG.event.type})
    if ischar(EEG.event(i).type)
        EEG.event(i).type=str2double(string(EEG.event(i).type));
    end
end