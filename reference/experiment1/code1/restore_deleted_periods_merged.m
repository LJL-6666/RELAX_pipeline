% 恢复删除时间段为NaN - 已合并的data-tongyong数据集
% 从Cleaned_Data_Merged读取合并后的文件，恢复删除时间段为NaN，保存到Cleaned_Data_Merged_Restored
%
% 输入：data/data-tongyong/原始数据/可用原始数据/转化成set的脑电_aligned/RELAXProcessed/Cleaned_Data_Merged/*_RELAX_merged.set
% 输出：data/data-tongyong/原始数据/可用原始数据/转化成set的脑电_aligned/RELAXProcessed/Cleaned_Data_Merged_Restored/*_RELAX_merged_restored.set
%
% 说明：
%   - 输入文件是按vid顺序合并后的文件（包含所有vid：vid01, vid02, ..., vid28）
%   - 每个vid的开始位置有事件标记（type='vid01', 'vid02', ...）
%   - RELAX.ExtremelyBadPeriodsForDeletion 记录了合并后时间轴上的删除时间段
%   - 本脚本将这些删除时间段恢复为NaN，保持合并后的数据结构

clear; clc;

% 设置路径
base_dir = '/data/liujialing/TY';
input_dir = fullfile(base_dir, 'data/data-tongyong/原始数据/可用原始数据/转化成set的脑电_aligned/RELAXProcessed/Cleaned_Data_Merged');
...
fprintf('============================================================\n');
fprintf('恢复删除时间段为NaN - 已合并的data-tongyong数据集\n');
fprintf('============================================================\n');
fprintf('输入目录: %s\n', input_dir);
fprintf('输出目录: %s\n\n', output_dir);

% 获取所有_RELAX_merged.set文件
files = dir(fullfile(input_dir, '*_RELAX_merged.set'));
if isempty(files)
    error('未找到任何_RELAX_merged.set文件');
end

fprintf('找到 %d 个合并后的文件\n\n', length(files));
...
for i = 1:length(files)
    ...
    try
        ...
        % 检查合并信息
        if isfield(EEG, 'merged_info')
            fprintf('  被试ID: %s\n', EEG.merged_info.subject_id);
            fprintf('  包含vid数: %d\n', EEG.merged_info.vid_count);
            fprintf('  vid列表: [%s]\n', num2str(EEG.merged_info.vid_list));
        end
        ...
        if isempty(bad_periods) || size(bad_periods, 1) == 0
            ...
            % 如果没有删除时间段，直接复制并重命名
            output_filename = strrep(input_filename, '_RELAX_merged.set', '_RELAX_merged_restored.set');
        ...
        % 显示删除时间段详情（前5个）
        fprintf('  删除时间段详情（显示前5个）:\n');
        for j = 1:min(5, size(bad_periods, 1))
            period_len = bad_periods(j, 2) - bad_periods(j, 1) + 1;
            fprintf('    [%d] 位置: %d-%d, 长度: %d 采样点 (%.2f秒)\n', ...
                j, bad_periods(j, 1), bad_periods(j, 2), period_len, period_len/EEG.srate);
        end
        ...
        if ~isempty(EEG.event)
            fprintf('  更新事件时间...\n');
            original_event_count = length(EEG.event);
            events_in_deleted_periods = 0;
            ...
            for e = 1:length(EEG.event)
                ...
                for j = 1:size(sorted_periods, 1)
                    ...
                    elseif sorted_periods(j, 1) <= original_latency && sorted_periods(j, 2) >= original_latency
                        ...
                        event_in_deleted = true;
                        events_in_deleted_periods = events_in_deleted_periods + 1;
                        break;
                    end
                end
                ...
                % 标记在删除时间段内的事件
                if event_in_deleted
                    if ~isfield(EEG.event(e), 'in_deleted_period')
                        EEG.event(e).in_deleted_period = true;
                    end
                end
            end

            fprintf('  事件总数: %d\n', original_event_count);
            if events_in_deleted_periods > 0
                fprintf('  警告: %d 个事件在删除时间段内\n', events_in_deleted_periods);
            end
        end

        % 更新merged_info（如果存在）
        if isfield(EEG, 'merged_info')
            EEG.merged_info.total_duration = EEG.pnts / EEG.srate;
            EEG.merged_info.restored = true;
            EEG.merged_info.deleted_periods_count = size(sorted_periods, 1);
            EEG.merged_info.total_deleted_samples = total_deleted;
        end
        ...
        non_nan_count = sum(~isnan(EEG.data(:)));
        fprintf('  数据验证: NaN数量=%d, 非NaN数量=%d\n', nan_count, non_nan_count);
