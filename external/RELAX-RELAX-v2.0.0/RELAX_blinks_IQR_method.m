...
% eye blinks in the continuous EEG trace
    ...
    UpperBound=squeeze(Upper25+(RELAX_cfg.BlinkDetectThreshould*InterQuartileRangeAllTimepointsAndEpochs)); % sets the threshold, above which a period is assumed to have a blink

    % Initialize BlinkIndexMetric to avoid undefined variable error
    BlinkIndexMetric = zeros(1, size(EEGEyeOnly.data,2));
