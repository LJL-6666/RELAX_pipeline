function aligned_files = align_by_vid_and_convert_to_set_multiple_bdf(...
    bdf_file_paths, video_segments, trial_vid_pairs, subject_id, output_dir, cap_file)
% 按vid对齐视频段并转换为SET格式（支持多个BDF文件）
% 输入：
%   bdf_file_paths: BDF文件路径的cell数组
...
try
    ...
    fprintf('  读取合并后的BDF数据...\n');
    cfg = [];
    cfg.headerfile = bdf_file_paths;  % 传入cell数组，FieldTrip会自动合并
    ...
    for vid_idx = 1:length(all_vids)
        ...
        % 读取段数据（使用合并后的BDF文件列表）
        % FieldTrip支持cell数组，会自动处理多个文件的合并
        % 注意：不传入header参数，让ft_read_data自己读取每个文件的header
        % 这样可以避免ft_setopt的cell数组参数问题
        data = ft_read_data(bdf_file_paths, ...
